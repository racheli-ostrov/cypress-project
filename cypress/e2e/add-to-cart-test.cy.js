describe('בדיקת הוספת מוצרים לעגלה', () => {
  ...
  it('בדיקה מקיפה: הוספת 3 מוצרים לעגלה ואימות בעגלה', () => {
    ...
    cy.log('\n✅ הטסט המקיף הושלם בהצלחה!');
    
    // יצירת קובץ אקסל עם תוצאות הבדיקה
    cy.task('generateExcel', 'cart-test-results.xlsx').then(() => {
      cy.log('\n📄 קובץ אקסל נוצר בהצלחה: output/cart-test-results.xlsx');
    });
  });
});
